...
fun main(){
    //TempoDeVida(), Triangulo(), TipoDeTriangulo(), jooKenPo(), adivinhaNumero()
...
fun TempoDeVida(){
    println("Quantos cigarros vc fuma por dia:")
    val qtd = readln().toInt()
    println("Por quanto tempo vc ja fumou:")
    val a = readln().toInt()
    val m = qtd * 10
    val h = m / 60
    val d = h / 24

    println("O fumante perdeu ${d * a} de vida")
}

fun Triangulo(){
    println("Digite o valor dos três segmentos:")
    val l1 = readln().toDouble()
    val l2 = readln().toDouble()
    val l3 = readln().toDouble()
...
fun TipoDeTriangulo() {
    ...
    if (l1 < l2 + l3 && l2 < l1 + l3 && l3 < l1 + l2) {
        ...
        when {
            l1 == l2 && l2 == l3 -> println("O triângulo é EQUILÁTERO.")
            l1 == l2 || l2 == l3 || l1 == l3 -> println("O triângulo é ISÓSCELES.")
